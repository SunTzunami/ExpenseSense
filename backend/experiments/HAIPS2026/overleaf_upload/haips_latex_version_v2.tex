...
with 1.2\,GB RAM.
Critically, dual-agent architectures shift errors from spurious parameters to safer,
missing parameters, reducing the risk of silent incorrect outputs.
...
\begin{enumerate}
    \item Empirical \textbf{on-device deployment guidelines for
          ...
          demonstrating that private, high-quality personal finance analytics
          can run locally on consumer hardware - making on-device
          inference a practical privacy protection feature.
    ...
    \item A \textbf{fine-grained error taxonomy} of 7 categories
          ...
          can mislead the user) toward missing parameters
...
\textbf{(2)} While single-agent prompts allow Qwen3.5-2B to
...
79.8\% to 89.1\% Task Accuracy (+9.3~pp) while almost halving latency
...
\begin{itemize}\itemsep=0pt \parskip=0pt \parsep=0pt
    \item All models use Q8\_0 quantisation; lower-bit formats
          ...
          tradeoffs.
    \item Test cases were authored by a single developer in English, reflecting a
          Japan-based context; crowdsourcing and multilingual support would improve diversity.
    \item Ground-truth labels are deterministic; some queries may
          admit legitimate alternative tool selections.
    \item We evaluate tool-call formation, not downstream financial calculation correctness.
    \item Our privacy guarantee is architectural (no data leaves the device) rather than cryptographic.
\end{itemize}
