...
\title{SpendSight: Privacy-Preserving Personal Finance Q\&A \\ with Sub-2B LLMs}
...
\begin{abstract}
People often speculate about their personal
finances: \emph{``Has my spending on groceries increased compared to past year?''},
\emph{``How does my spending on clothes look for the past 6 months?''}. However, answering them
with cloud-hosted LLMs transmits sensitive financial data to external servers, exposing users to data leakage and inferential privacy risks.
We present \textbf{SpendSight}, a study of fully on-device
personal finance Q\&A tool calling that keeps all data strictly local.
We evaluate seven quantized models (six sub-2B and one 3B) across
115 test cases and five analytical tools, comparing single-agent
against dual-agent (router-specialist) architectures.
For sub-2B models, dual-agent decomposition yields substantial
gains: Qwen3.5-0.8B achieves 89.1\% Task Accuracy in 3.0~s with
1.2\,GB RAM.
A 3B reference model (G9v3-3B, Q4\_K\_M) achieves 90.6\%
single-agent Task Accuracy at 2.3~s, suggesting that larger
models with aggressive quantization can bypass decomposition.
A real-world case study confirms practical impact: a user
reduced snack spending by 84.7\%
(\textyen38,453 $\to$ \textyen5,893) after insights surfaced
entirely on-device.
\end{abstract}
...
People often wonder about their personal finances.
\emph{Has my spending on dining increased month over month?}
\emph{What were some of my largest purchases last month?}
These are natural, everyday questions, and modern large language
models (LLMs) can answer them by calling structured analytical
tools over a user's expense database
\citep{schick2024toolformer, patil2023gorilla, qin2023toolllm}.

The problem is \emph{where} this inference happens.
Cloud-hosted assistants require users to transmit their financial
queries, and often the underlying data, to external servers.
A query like ``how much did I spend on therapy last month?'' is
not merely a database lookup; it is a disclosure of a health
condition, a temporal pattern, and a financial commitment.
Financial records encode sensitive behavioral patterns that risk data leakage and inferential privacy violations when processed remotely \citep{staab2024memorization, yao2024survey}.

This creates an underexplored problem at the intersection of
financial NLP and on-device inference: \emph{can sub-2B quantized
models running entirely on consumer hardware reliably call
domain-specific financial tools from informal, colloquial
natural language?}
If so, on-device inference becomes not merely a technical
curiosity but a practical privacy guarantee for real users, one
that eliminates cloud dependency entirely.

Existing work on financial tool calling, such as FinMCP-Bench
\citep{finmcpbench2026} and FinTrace
\citep{fintrace2026}, evaluates cloud-scale models ($\geq$7B)
on multi-step trajectories.
General-purpose on-device models like Octopus~v2
\citep{chen2024octopus} and Hammer \citep{lin2024hammer}
demonstrate the potential of smaller models but have not
been evaluated on domain-specific financial schemas or
under consumer-hardware latency constraints.
Our study bridges this gap.

We address three research questions (\textbf{RQ}):

\begin{itemize}\itemsep=0pt \parskip=0pt \parsep=0pt
  \item \textbf{RQ1}: Can on-device inference with sub-2B models
        provide practically viable, private personal finance
        analytics (i.e., can users obtain accurate personal finance
        insights without sending query data to cloud providers)?
  \item \textbf{RQ2}: Does decomposing intent classification from
        parameter extraction (dual-agent) yield significant
        accuracy gains over a joint single-agent architecture?
  \item \textbf{RQ3}: What are the primary failure modes across
        architectures, and how do they differ in the real-world
        harms they produce for users relying on financial outputs?
\end{itemize}

This paper presents the following contributions:

\begin{enumerate}
    \item A \textbf{study of on-device financial tool calling}:
          Qwen3.5-0.8B dual achieves 89.1\% Task Accuracy at
          3.0~s latency and $\approx$1.2\,GB RAM, keeping all
          data strictly local.

    \item A \textbf{fine-grained error taxonomy} revealing that
          dual-agent architectures shift errors from silent
          parameter corruption toward visible omissions, a
          qualitative safety improvement for financial outputs.

    \item A \textbf{comparative evaluation} of single-agent
          vs.\ dual-agent across six sub-2B quantized models
          (0.8B--2.0B), showing dual-agent reduces latency
          (up to 48\%), improves exact-match correctness
          (+17.4~pp CR), and stabilises sub-1B models.

    \item An \textbf{extended comparison} with a 3B model
          (G9v3-3B, Q4\_K\_M) showing that larger models under
          aggressive quantization can match sub-2B dual-agent
          accuracy in single-agent mode, delineating when
          decomposition is necessary.

    \item A \textbf{real-world case study}: on-device insights
          led a user to reduce snack spending by 84.7\% over
          six months, with no data leaving the device.
\end{enumerate}

% ─────────────────────────────────────────────────────────────────────────────
\section{On-Device Inference as Privacy Protection}
\label{sec:privacy}
% ─────────────────────────────────────────────────────────────────────────────

Conversational interfaces for personal finance expose a structural issue: routing natural-language queries through cloud LLMs leaks behavioral signals that can be retained, aggregated, and exploited \citep{staab2024memorization, yao2024survey}.
SpendSight addresses this threat by executing all inference \emph{on-device}. By leveraging sub-2B quantized models that run efficiently on consumer hardware (tested on an Apple M1 with 8\,GB memory), all user data, queries, and model outputs remain strictly local, preserving \emph{contextual integrity} \citep{nissenbaum2004privacy}.

The threat model encompasses cloud data retention, query-level inference attacks, and third-party data exposure. On-device inference eliminates all three by ensuring queries never leave local infrastructure. While sub-2B models have historically been unreliable for structured tool calling \citep{kavathekar2025small}, our study demonstrates that architectural decomposition (separating intent classification from parameter extraction) bridges this reliability gap, making private on-device inference practical for real personal financial Q\&A systems.
...
AVC, Task Accuracy, and Correct Ratio) for small model
function calling; we adopt these metrics directly.
CallNavi \citep{callnavi2025} proposed a two-stage
router+specialist approach, similar to our dual-agent
design, but evaluates models $\geq$7B on general-purpose
APIs.
\citet{jhandi2025slafc} studied fine-tuning SLMs for
agentic tool calling using trajectory-level data,
demonstrating that targeted domain adaptation can
close the gap with larger models.
In contrast, we focus on sub-2B models for domain-specific,
single-turn personal finance tasks without fine-tuning.
...
calling at scale with a $\sim$3B model.
\citet{kavathekar2025small} found that structured output compliance
is the primary bottleneck for SLMs.

\paragraph{Financial NLP and Tool Calling.}
FinQA \citep{chen2021finqa} and FinGPT \citep{yang2023fingpt} target
financial numerical reasoning and generation but focus on text
generation rather than structured tool invocation.
More recently, FinMCP-Bench \citep{finmcpbench2026} evaluates LLM
agents for real-world financial tool use under the Model Context
Protocol, and FinTrace \citep{fintrace2026} provides trajectory-level
evaluation of tool calling for long-horizon financial tasks.
Both target cloud-scale models ($\geq$7B) on multi-step
trajectories involving dozens of financial APIs.
Our study is complementary: we focus on \emph{single-turn,
on-device} tool calling with sub-2B models for personal
finance, a setting where privacy constraints make cloud
inference undesirable and where the user directly consumes
the output for personal decision-making.
Personal finance assistants \citep{li2024personal} require strict
privacy, making local execution crucial.

\paragraph{Privacy for Financial Language Models.}
Contextual integrity \citep{nissenbaum2004privacy} dictates that personal financial data should not cross contextual norms. Since LLMs can reconstruct sensitive attributes from text \citep{staab2024memorization}, on-device processing is critical to eliminate cloud exposure pathways entirely.
...
SpendSight exposes five analytical tools over a user's expense
database.
Table~\ref{tab:tools} summarises each tool and its parameter space.
These tools span core personal finance use cases that reflect the
questions users naturally ask: time series analysis (\emph{``how
has my spending changed?''}), categorical breakdown (\emph{``how 
does the breakdown of my spending look?''}), period comparison (\emph{``how
does this month compare to last?''}), aggregation (\emph{``how
much did I spend on X?''}), and ranked retrieval (\emph{``what
were my biggest purchases?''}).
...
The evaluation comprises 115 test cases across five task groups
...
\emph{``shinkansen''}).
This mimics how real users interact with personal finance
assistants, using casual language with abbreviations and
cultural terminology specific to their context.
...
We adopt the metric suite of \citet{kavathekar2025small} to assess tool routing and parameter extraction performance:

\begin{itemize}
    \item \textbf{FSP} (Function Selection Precision): Binary
          indicator of correct tool prediction (i.e.\ routing).
    \item \textbf{ACS} (Argument Completeness Score): F$_1$ on
          argument \emph{name} overlap (ignoring values).
    \item \textbf{AVC} (Argument Value Correctness): Fraction of
          correctly named arguments whose extracted values also match.
    ...
    \item \textbf{Correct Ratio (CR)}: Strict binary indicator: 1.0
          ...
          is perfectly correct for execution.
\end{itemize}

All metrics are averaged across 5 repetitions per test case
(temperature $= 0.1$). 95\% bootstrap confidence intervals
\citep{efron1979bootstrap} ($n=2000$ resamples) were computed
for statistical testing of the overall gap between models,
...
This taxonomy is crucial for evaluating the \emph{safety profile}
of on-device financial assistants: in personal finance, a
silently wrong answer (from a spurious parameter) is more
harmful than a visibly incomplete one (from a missing parameter).
...
\midrule
G9v3-3B$^\dagger$ & G9v3 \citeyearpar{ai9stars2026g9v33b}  & 3B   & 2.4\,GB \\
\bottomrule
\end{tabular}
\caption{Models evaluated. Sub-2B models use Q8\_0 GGUF
quantization; $^\dagger$G9v3-3B uses Q4\_K\_M. All run via
llama.cpp. RAM is average peak RSS.}
\label{tab:models}
\end{table}

All sub-2B models (Table~\ref{tab:models}) use Q8\_0 GGUF
quantization; G9v3-3B uses Q4\_K\_M. All seven models run via
llama.cpp \citep{llamacpp2023} with full GPU offload on Apple Metal.
G9v3-3B was selected as the top-ranked open-weight model under 4B
parameters on the Artificial Analysis Intelligence Index at the
time of evaluation. Since G9v3-3B supports an optional extended
thinking mode, we disabled it (\texttt{enable\_thinking=false}) for
all runs, to keep it comparable to the non-reasoning inference
setting used for the other six models.
...
yielding $115\!\times\!5\!\times\!7\!\times\!2 = 8{,}050$ total
observations.
...
Figure~\ref{fig:overall_visuals} shows the accuracy-latency
trade-off and the task footprint across architectures.
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    \caption{Accuracy-latency trade-off. Dual-mode
    Qwen3.5-0.8B ($\approx$3~s, 89\%) sits on the
...
\midrule
\multirow{7}{*}{\rotatebox{90}{Single}}
& EXAONE-4.0-1.2B          & 19.3 & N/A  & 0.3  & 8.9  & 0.3  & 0.1  & 2.0 \\
& Gemma-3-1B-it            & 27.0 & N/A  & 47.5 & 30.0 & 0.9  & 0.8  & 1.6 \\
& LFM2.5-1.2B              & 28.7 & N/A  & 11.7 & 17.2 & 3.1  & 7.8  & 1.4 \\
& MiniCPM5-1B              & 71.1 & N/A  & 86.7 & 62.3 & 13.0 & 2.6  & 1.5 \\
& Qwen3.5-0.8B             & 95.0 & N/A  & 91.2 & 79.8 & 40.2 & 5.8  & 1.3 \\
& \textbf{Qwen3.5-2B}     & \textbf{96.5} & N/A & \textbf{97.6} & \textbf{92.0} & 60.2 & 12.0 & 2.1 \\
\cmidrule{2-9}
& G9v3-3B$^\dagger$        & 95.7 & N/A  & 97.0 & 90.6 & \textbf{71.7} & 2.3  & 2.4 \\
\midrule
\multirow{7}{*}{\rotatebox{90}{Dual}}
...
& \textbf{Qwen3.5-2B}     & 92.0 & 91.6 & \textbf{97.4} & \textbf{90.4} & \textbf{77.6} & 6.2 & 2.1 \\
\cmidrule{2-9}
& G9v3-3B$^\dagger$        & 92.2 & 89.7 & 95.8 & 88.2 & 68.3 & 10.2 & 2.3 \\
\bottomrule
\end{tabular}
\caption{Main results (validated metrics, \%). \textbf{Bold} = best
per metric within each mode. FSP = Function Selection Precision;
ACS = Argument Completeness Score; AVC = Argument Value Correctness;
Task Acc = F$_1$-based task accuracy; CR = strict exact-match
Correct Ratio. ACS is omitted (N/A) for single-agent configurations.
$^\dagger$3B model (Q4\_K\_M), included as a reference point
outside the core sub-2B study. Latency and RAM are per-query
averages.}
\label{tab:main_results}
\end{table*}

\paragraph{Key findings (RQ1, RQ2).}
\textbf{(1)} Sub-2B models can reliably call financial tools
on consumer hardware (\textbf{RQ1}), with the Qwen3.5 family
achieving $>$89\% Task Accuracy in dual mode.
This demonstrates that users can obtain accurate financial
analytics, from spending trends to category breakdowns, without
any data leaving their device.
\textbf{(2)} While single-agent Qwen3.5-2B reaches 92.0\% Task
Accuracy, dual-agent decomposition is preferred for two reasons
(\textbf{RQ2}):
...
financial outputs a user might act on.
\emph{(b)} Enabling smaller models: Qwen3.5-0.8B improves from
79.8\% to 89.1\% Task Accuracy (+9.3~pp) while almost halving
latency (5.8~s to 3.0~s), making sub-1B models viable for
low-memory devices.
\textbf{(3)} The 3B reference model G9v3-3B is the sole model
where single-agent outperforms dual-agent (90.6\% vs.\ 88.2\%
Task Accuracy; 71.7\% vs.\ 68.3\% CR), indicating a capacity
threshold beyond which decomposition provides diminishing returns.

\subsection{Per-Tool-Group Analysis}

Table~\ref{tab:category_results} shows per-group accuracy for the top
two dual-mode models (see Figure~\ref{fig:category_heatmap} in the Appendix for the full model $\times$ category heatmap).
\texttt{get\_top\_expenses} reaches 98.9\% accuracy with Qwen3.5-2B
due to its simple parameter structure, aligning with its role as one of
the most common user questions (``what were my biggest
purchases?'').
...
temporal reasoning.
G9v3-3B dual follows a similar pattern, achieving 96.1\% on
distribution but only 67.4\% on time series.
...
Task Accuracy decreases from 96\% (L1) to 86\% (L3) for
Qwen3.5-2B, and from 91\% to 80\% for Qwen3.5-0.8B.
G9v3-3B dual degrades similarly (96\% L1, 82\% L3).
L3 queries involve constructs like cross-year range comparisons
(\emph{``compare groceries nov 2024 to april 2025 vs nov 2025
to april 2026''}), which require extracting up to 10 parameters
with complex temporal constraints, exactly the kind of
year-over-year analysis users commonly want.
...
Figure~\ref{fig:error_plot} (Appendix) visualises the full error distribution.
...
at 8.0\%.
This shift matters for financial safety:
a \texttt{PARAM\_EXTRA} error silently restricts scope (e.g., adding
a spurious \texttt{months=1} returns last month's total instead of
all-time), producing a plausible but wrong answer.
A \texttt{PARAM\_MISSING} error yields a visibly broader result,
more likely to prompt user scrutiny.
The dual-agent shift from extra to missing parameters thus
represents a qualitative safety improvement.
Qwen3.5-0.8B shows a similar pattern: single-agent produces
32.5\% \texttt{PARAM\_MISSING} / 11.8\% \texttt{PARAM\_EXTRA};
dual-agent reduces these to 12.9\% / 11.1\%.
G9v3-3B is a notable exception: \texttt{PARAM\_EXTRA} remains
at $\approx$11\% in both architectures (11.1\% single, 11.8\%
dual), indicating that the dual-agent error-reduction benefit
does not extend to larger models where single-agent capacity is
already sufficient.

\subsection{Token Efficiency and Validation}

Despite using $\approx$2,700 prompt tokens per query (vs.\
$\approx$5,500 for single-agent), the dual-agent architecture
achieves equivalent or superior accuracy
(see Figure~\ref{fig:token_efficiency} in the Appendix).
The single-agent pipeline must simultaneously attend to all five
tool schemas, while the dual-agent decomposes the context, focusing
each pass on a single subtask, achieving higher accuracy per
token, an important property for memory-constrained on-device
deployment.
The post-hoc validation layer (Section~\ref{sec:validation})
contributes modest but consistent gains (up to +1.2~pp Task
Accuracy for MiniCPM5-1B), primarily correcting category casing and
morphological normalisation.
This confirms that argument extraction, not category matching,
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{26_beginning_image.png}
    \caption{\textbf{Alert} (Jan 2026): Querying snack spending over past 6 months reveals 63 transactions (\textyen30,088) with peaks near \textyen1,000, prompting a decision to cut back.}
    \label{fig:case_study_alert}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{26_first_6_months_vs25_last_6_months.png}
    \caption{\textbf{Confirmation} (June 2026): Period comparison confirms an 84.7\% reduction in snack spending (\textyen38,453 $\to$ \textyen5,893). Higher average transaction (\textyen506 $\to$ \textyen655) confirms fewer purchases were the main reason for reduction in spending.}
    \label{fig:case_study_confirm}
  \end{subfigure}
  \caption{Real-world deployment: private on-device analytics driving behaviour change. Dual-agent Qwen3.5-0.8B (3.3~s) and Qwen3.5-2B (8.7~s) correctly route queries locally. The participant reduced snack spending by 84.7\%. Intermediate views in Figures~\ref{fig:case_mid}--\ref{fig:case_year} (Appendix).}
  \label{fig:case_study}
\end{figure*}

\paragraph{Privacy through Task Decomposition.}
The dual-agent architecture improves accuracy and makes local inference viable.
By separating routing (single digit) from extraction (focused JSON), dual-agent reduces generation complexity per call.
Decomposing the task helps sub-2B models perform reliably on domain-specific financial schemas, making on-device inference a practical privacy guarantee.
The exception is G9v3-3B (3B), where single-agent already matches dual-agent performance, indicating a capacity threshold where decomposition becomes unnecessary.

\paragraph{Comparison with Cloud-Scale Tool Calling.}
Our study is complementary to cloud-focused financial evaluations like FinMCP-Bench \citep{finmcpbench2026} and FinTrace \citep{fintrace2026}, which evaluate $\geq$7B models on multi-step trajectories.
SpendSight targets single-turn, on-device, privacy-preserving tool calling where latency must be minimal and no data leaves the device.
Our finding that sub-2B models achieve $>$89\% Task Accuracy suggests cloud-scale models may not be necessary for single-turn personal financial queries.

\paragraph{Model Scale vs.\ Quantization.}
G9v3-3B (3B, Q4\_K\_M, 2.4\,GB) demonstrates an alternative path: a larger model under aggressive quantization.
Its 90.6\% single-agent Task Accuracy matches sub-2B dual-agent accuracy at 2.3~s latency.
However, dual-agent G9v3-3B incurs 10.2~s latency without error-profile improvements.
These results suggest a practical threshold around 2--3B parameters beyond which single-agent capacity suffices.

\paragraph{Addressing Highly Complex Queries.}
Even the best model achieves 59.6\% strict Correct Ratio on L3 queries (Figure~\ref{fig:difficulty_plot}), which involve complex cross-year comparisons requiring up to 10 parameters.
Fine-tuning on domain-specific synthetic data \citep{liu2024apigen} could improve performance on L3 queries while preserving on-device deployment.

\paragraph{Real-World Impact: From Analytics to Behaviour Change.}
Figure~\ref{fig:case_study} illustrates how on-device analytics drive financial behaviour change.
In January~2026, a user queried ``\emph{can ya visualize spend on snacks for past 6 mos?}'' (Figure~\ref{fig:case_study_alert}); Qwen3.5-0.8B dual routed this to \texttt{plot\_time\_series} in 3.3~s, revealing 63~transactions (\textyen30,088) with peaks near \textyen1,000, serving as an alert to cut back.
Six months later, the user verified the impact via ``\emph{compare spend on snacks jul -- dec 2025 vs jan -- jun 2026}'' (Figure~\ref{fig:case_study_confirm}); Qwen3.5-2B dual produced the result in 8.7~s, confirming an \textbf{84.7\% reduction} (\textyen38,453 $\to$ \textyen5,893).\footnote{Totals vary slightly between single-period charts and dual-period comparisons due to rolling vs.\ calendar windows.}
The per-transaction average increased (\textyen506 $\to$ \textyen655), confirming fewer purchases resulted in decrease in overall spending.
Crucially, all interactions remained strictly on-device.
...
We presented SpendSight, a study of privacy-preserving on-device personal finance Q\&A tool calling with quantized LLMs.
Local inference at this scale is practically reliable: Qwen3.5-0.8B dual achieves 89.1\% Task Accuracy at 3.0~s latency on-device.
Dual-agent decomposition improves sub-2B strict correctness (60.2\% $\to$ 77.6\% CR) and shifts errors toward safer \texttt{PARAM\_MISSING} failures, while G9v3-3B reaches 90.6\% Task Accuracy without decomposition.
A real-world case study confirms tangible impact: a participant reduced snack spending by 84.7\% from on-device insights.
We recommend Qwen3.5-0.8B dual for memory-constrained settings ($\leq$2\,GB), Qwen3.5-2B dual for accuracy-first use (77.6\% CR), and G9v3-3B single for minimal-latency single-pass deployment (71.7\% CR at 2.3~s).
...
(1)~Sub-2B models use Q8\_0 quantisation; G9v3-3B uses Q4\_K\_M. The interaction between model scale and quantisation warrants further study.
(2)~Test cases were authored by a single developer in English reflecting a Japan-based context.
(3)~Ground-truth labels are deterministic.
(4)~Privacy guarantee is architectural rather than cryptographic.
...
SpendSight preserves user financial privacy via on-device inference.
The category taxonomy (Appendix~\ref{sec:category_vocabulary}) and case study (Section~\ref{sec:discussion}) derive from author self-tracked expense data, used with consent.
Evaluation queries are synthetic; no real transaction amounts or records are in the test suite.
No third-party data was used.
The system complements professional financial advice.
We advocate for clear user disclaimers in production.
...
Code, prompts, evaluation data, and results will be released upon acceptance via a github repository.
Artefacts include: 115 test cases with ground-truth labels, router/specialist templates, inference configs, result CSVs, evaluation runner, plotting scripts, and model GGUFs for complete reproducibility.
...
Figure~\ref{fig:error_plot} visualises the full error distribution breakdown. Figure~\ref{fig:category_heatmap} presents the model $\times$ category Task Accuracy heatmap. Figure~\ref{fig:combined_model_radar_grid} shows the per-category footprint across all evaluated models. Figure~\ref{fig:validation_delta_heatmap} illustrates the metrics improvement provided by the post-hoc validation layer. Figure~\ref{fig:token_efficiency} shows the token efficiency trade-off between architectures. Figures~\ref{fig:case_mid} and~\ref{fig:case_year} provide intermediate views of the real-world case study (Section~\ref{sec:discussion}).

\begin{figure*}[htbp]
  \centering
  \includegraphics[width=\textwidth]{error_taxonomy_breakdown.png}
  \caption{Error taxonomy breakdown (\% of test cases) per model and architecture. Single-agent Qwen3.5-2B produces 22.6\% \texttt{PARAM\_EXTRA} errors (spurious parameters that could lead to silently incorrect financial outputs). Dual-agent eliminates most of these, consolidating residual errors into the more predictable \texttt{PARAM\_MISSING} category.}
  \label{fig:error_plot}
\end{figure*}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{dual_category_heatmap.png}
  \caption{Task Accuracy (\%) by model $\times$ tool category in dual mode. Qwen3.5 models achieve $>$74\% across all categories, while weaker models show marked gaps in comparison and time series tasks.}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{token_efficiency_scatter.png}
  \caption{Token efficiency: Task Accuracy vs.\ average prompt tokens
  per query. Dual-agent configurations (red squares) achieve higher
  accuracy with fewer total tokens than their single-agent counterparts
  (blue circles), demonstrating superior context utilisation.}
  \label{fig:token_efficiency}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{26_mid_image.png}
  \caption{Case study intermediate view: six months \emph{after} the
  participant decided to reduce snack spending (queried June~2026).
  Only 18 transactions totalling \textyen8,584 remain over six months,
  with long gaps between purchases, in stark contrast to the dense
  pattern in Figure~\ref{fig:case_study_alert}.}
  \label{fig:case_mid}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\textwidth]{26_1_year_ago.png}
  \caption{Case study full-year view: weekly snack spending over the
  past year (queried June~2026). The 4-week moving average (dotted
  line) shows a clear downward trend from peaks of \textyen3,000+ in
  mid-2025 to near-zero by early 2026, illustrating the sustained
  behaviour change following the initial alert in January~2026.}
  \label{fig:case_year}
\end{figure}
...
The on-device prompts utilised for routing and parameter extraction tasks are detailed below.
