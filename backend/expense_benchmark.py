...
# -----------------------------------------------------------------------------
from experiments.shared.inference import generate, get_last_usage
from experiments.configs.models import get_llamacpp_models
from experiments.shared.memory import free_model_memory
...
1. plot_time_series(df, category=None, year=None, start_year=None, start_month=None, end_year=None, end_month=None, months=None)

2. plot_distribution(df, category=None, major_category=None, remarks=None, year=None, month=None, start_year=None, start_month=None, end_year=None, end_month=None, months=None, ignore_rent=False)

3. plot_comparison_bars(df, category=None, major_category=None, y1=None, m1=None, d1=None, y2=None, m2=None, d2=None)
...
CATEGORY_MAPPING = CATEGORIES.get("CATEGORY_MAPPING", {})
...
# Import TEST_CASES from backend/test_cases.py explicitly (not experiments/configs/test_cases.py)
import importlib.util as _ilu
_tc_spec = _ilu.spec_from_file_location("backend_test_cases", os.path.join(BACKEND_ROOT, "test_cases.py"))
...
def score_row(
    ...
    gt_params = expected_params or {}
    pred_params = predicted_params or {}

    # FSP
    fsp = 1.0 if str(predicted_tool).strip().lower() == str(expected_tool).strip().lower() else 0.0

    # ACS — F1 on argument name overlap.
    # FIX: exclude the positional 'df' argument from predicted keys; it is always
    # present in parsed output but never in the ground-truth expected dict.
    gt_arg_names = {k.lower() for k in gt_params.keys()}
    pred_arg_names = {k.lower() for k in pred_params.keys() if k.lower() != "df"}
...
def classify_error(
    ...
    gt_keys = {k.lower() for k in (expected_params or {}).keys()}
    # FIX: exclude 'df' from predicted keys before error classification
    pred_keys = {k.lower() for k in (predicted_params or {}).keys() if k.lower() != "df"}
    ...
    for key in gt_keys & pred_keys:
        gt_val = str(expected_params.get(key, "")).strip().lower()
        pred_val = str(predicted_params.get(key, "")).strip().lower()
        if gt_val and pred_val and gt_val != pred_val:
